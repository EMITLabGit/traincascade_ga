...
import ga_feature_selection as ga_fs

logger = logging.getLogger(__name__)

# TODO:
# 1. Check the implementation of the various training functions to see if any updates are required to be compatible with the new normfactors
# 2. Define the implementation to select the best threshold
# 3. Check the implementation of update_weights to see if any updates are required to be compatible with the new normfactors
...
class CascadeClassifier:
    ...
    def _initialize_test_args(self):
        args = self.args
        ...
        # initialize the test data reader
        self.test_reader = data_reader.TestDataReader(args)
    ...
    def get_cascade_classifier(self, stage=None):
        ...
        if stage == 0:
            # self.wc_classifiers_all = [np.array([0, 0, 0, 0], dtype=np.int32)]
            # self.wc_thresholds_all = [0.0]
            # self.wc_polarities_all = [0]
            # self.wc_alphas_all = [0.0]
            # self.thresholds_stage = [0]
            # self.start_idx_stage = [0]
            # self.end_idx_stage = [0]
            # stage = 1
            return np.array(
    ...
    def update_training_data(self):
        ...
        # define neg_int_all, neg_int_sq_all, neg_nf_all which store the negative samples
        # neg_int_all = np.empty((0, self.args.H + 1, self.args.W + 1),
        #                        dtype=np.uint64)
        # neg_int_sq_all = np.empty((0, self.args.H + 1, self.args.W + 1),
        #                           dtype=np.uint64)
        # neg_nf_all = np.empty((0), dtype=np.float32)

        # neg_consumed = 0
        # neg_sample_count = 0
        # while neg_sample_count < self.args.numNeg:
        #     # first get a batch of negative data
        #     neg_int, neg_int_sq, neg_nf = self.data_reader.get_neg_data()
        #     neg_consumed += len(neg_int)
        #     # keep only the false positives from the negative data to generate hard negatives
        #     neg_int, neg_int_sq, neg_nf = self._filter_incorrect_preds(
        #         neg_int, neg_int_sq, neg_nf, keep_pred=1)
        #     neg_sample_count += len(neg_int)

        #     # concatenate neg_int to neg_int_all
        #     neg_int_all = np.concatenate(
        #         (neg_int_all,
        #          neg_int), axis=0) if neg_sample_count > 0 else neg_int_all
        #     neg_int_sq_all = np.concatenate(
        #         (neg_int_sq_all, neg_int_sq),
        #         axis=0) if neg_sample_count > 0 else neg_int_sq_all
        #     neg_nf_all = np.concatenate(
        #         (neg_nf_all,
        #          neg_nf), axis=0) if neg_sample_count > 0 else neg_nf_all

        # # keep only the first numNeg negative samples
        # neg_int_all = neg_int_all[:self.args.numNeg]
        # neg_int_sq_all = neg_int_sq_all[:self.args.numNeg]
        # neg_nf_all = neg_nf_all[:self.args.numNeg]

        neg_int_all, neg_int_sq_all, neg_nf_all, neg_consumed = self.data_reader.get_neg_data_3(
            self.args.numNeg)
        # return
        # get the latest cascade classifier
        # wc_classifiers, wc_thresholds, wc_polarities, wc_alphas, thresholds_stage, start_idx_stage, end_idx_stage = self.get_cascade_classifier(
        ...
        # # compute the cascade classifier output for the positive samples
        # preds, feat_vals, out_hypotheses = compute_cascade_classifier(
        #     neg_int_all, neg_nf_all, wc_classifiers, wc_thresholds,
        #     wc_polarities, wc_alphas, thresholds_stage, start_idx_stage,
        #     end_idx_stage)

        # # loop through each sample where preds is equal to 0 and call the scan_image_copy function to debug
        # for n in range(len(preds)):
        #     # calculate the integral image for the current gray image and its square
        #     img_gray = img_gray_all[n]
        #     img_gray_sq = utils.square(img_gray)
        #     int_im = utils.to_integral_numba(img_gray)
        #     int_im_sq = utils.to_integral_numba(img_gray_sq)
        #     x = windows_all[n][0]
        #     y = windows_all[n][1]
        #     feat_vals_i, nf_i, out_hypotheses_i = _scan_image_copy(
        #         wc_classifiers, wc_thresholds, wc_polarities, wc_alphas,
        #         thresholds_stage, start_idx_stage, end_idx_stage,
        #         img_gray_all[n], int_im, int_im_sq, self.args.W, H_all[n],
        #         W_all[n], self.args.W, True, x, y)

        #     if preds[n] == 0:
        #         data_reader._scan_image_2_python(
        #             wc_classifiers, wc_thresholds, wc_polarities, wc_alphas,
        #             thresholds_stage, start_idx_stage, end_idx_stage, img_gray,
        #             int_im, int_im_sq, self.args.W, H_all[n], W_all[n],
        #             self.args.W, True)
    ...
    def _train_stage_ga(self):
        ...
        # Step 3: Use GA to select the best features
        self.features, _ = ga_fs.select_features(img_int, nf, y, self.args,
                                              self._get_stage(),
                                              self.ga_config)

        # Step 4: Precalculate the feature values for the training data set
        fts = ft.compute_features_all_imgs(self.features, img_int, nf)
        sorted_indices = argsort_features(fts)
    ...
    def train_ga(self):
        ...
        for stage in range(start_stage, self.args.numStages):
            ...
            # Step 3: Train the weak classifiers using GA + AdaBoost
            self._train_stage_ga()
            logger.info(f"Stage {stage} trained")
            train_time = time.time() - start_time - data_gen_time
    ...
    def set_stage(self, stage):
        wc_classifiers, wc_thresholds, wc_polarities, wc_alphas, thresholds_stage, start_idx_stage, end_idx_stage = self.get_cascade_classifier(
            stage)
        self.wc_classifiers_all = wc_classifiers
        self.wc_thresholds_all = wc_thresholds
        self.wc_polarities_all = wc_polarities
        self.wc_alphas_all = wc_alphas
        self.thresholds_stage = thresholds_stage
        self.start_idx_stage = start_idx_stage
        self.end_idx_stage = end_idx_stage

    def train_stage_exp(self, features):
        # train the current stage of the cascade classifier using AdaBoost and the given features only for one stage
        ...
        # Step 3: Precalculate the feature values for the training data set
        fts = ft.compute_features_all_imgs(features, img_int, nf)
        sorted_indices = argsort_features(fts)
        ...
        return num_weak_clfs

    def exp_ga_pop_size(self):
        # Set the stage to 15
        stage = 15
        self.set_stage(stage)

        # Set the last_neg_gen_idx to 4000
        self.data_reader.last_neg_gen_idx = 4000
        ...
        features, _ = ga_fs.select_features(img_int, nf, y, self.args,
                                            self._get_stage(), self.ga_config)
        _ = self.train_stage_exp(features)

        n_estimators = self.args.maxWeakCount
        early_stopping_rounds = max(5, int(n_estimators * 0.1))

        run_times = []
        weak_clf_counts = []
        fitness_scores_avg_topk = []
        pop_sizes = []
        run_indices = []

        # loop through the population sizes
        for pop_size in self.args.popSizes:
            # loop through number of runs for each population size
            # set the population size
            self.ga_config.FEATURES_PER_GENERATION = pop_size
            self.ga_config.NUM_PARENTS_MATING = pop_size

            for run in range(self.args.numRuns):
                # initialize the GA feature selection

                # select features and measure the time taken to select the features
                start_time = time.time()
                features, ga_instance = ga_fs.select_features(
                    img_int, nf, y, self.args, self._get_stage(),
                    self.ga_config)

                # Train a classifier using the selected features
                num_weak_clfs = self.train_stage_exp(features)

                end_time = time.time()
                run_times.append(end_time - start_time)
                weak_clf_counts.append(num_weak_clfs)
                fitness_scores_avg_topk.append(
                    ga_instance.fitness_store.get_avg_score(50))

            # store the number of generations and the run indices
            pop_sizes.extend([pop_size] * self.args.numRuns)
            run_indices.extend(list(range(self.args.numRuns)))
        ...
        results_df.to_csv(os.path.join(self.args.results_dir,
                                       f"{exp_name}.csv"),
                          index=False)

    def exp_ga_num_generations(self):
        # Set the stage to 15
        stage = 15
        self.set_stage(stage)
        ...
        for num_gen in self.args.numGens:
            # loop through number of runs for each num_gen
            # set the number of generations
            self.ga_config.NUM_GENERATIONS = num_gen
            ...
            # store the number of generations and the run indices
            num_gens.extend([num_gen] * self.args.numRuns)
            run_indices.extend(list(range(self.args.numRuns)))
    ...
    def exp_ga_num_estimators(self):
        # Set the stage to 15
        stage = 15
        self.set_stage(stage)
        ...
        for num_estimators in self.args.numEstimators:
            # loop through number of runs for each num_estimators
            ...
            for run in range(self.args.numRuns):
                ...
                # select features and measure the time taken to select the features
                start_time = time.time()
                self.args.maxWeakCount = num_estimators
                features, ga_instance = ga_fs.select_features(
                    img_int, nf, y, self.args, self._get_stage(),
                    self.ga_config)

                # Train a classifier using the selected features
                self.args.maxWeakCount = 200
                num_weak_clfs = self.train_stage_exp(features)
            ...
            # store the number of generations and the run indices
            n_estimators.extend([num_estimators] * self.args.numRuns)
            run_indices.extend(list(range(self.args.numRuns)))
