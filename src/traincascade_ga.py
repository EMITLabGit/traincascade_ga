...
if __name__ == '__main__':
    ...
    # Setup the GA configuration
    ga_config = GA_config()
    ga_config.SIZE = 5
    ga_config.NUM_GENERATIONS = 150
    ga_config.NUM_PARENTS_MATING = 100
    ga_config.FEATURES_PER_GENERATION = 100
    ga_config.KEEP_ELITISM = 1
    ga_config.PARENT_SELECTION_TYPE = "rws"
    ga_config.CROSSOVER_TYPE = "scattered"
    ga_config.MUTATION_PROBABILITY = 0.2
    args.LGBM_NUM_ESTIMATORS = 50

    # Setup logging
    setup_logging(args.log_file)

    cascade_clf = cascade_classifier.CascadeClassifier(args)
    cascade_clf.ga_config = ga_config
    cascade_clf.train_ga()